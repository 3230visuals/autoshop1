...
const StageChangeModal: React.FC<StageChangeModalProps> = ({ isOpen, targetStageIndex, onConfirm, onCancel }) => {
    return (
        <AnimatePresence>
            {isOpen && targetStageIndex !== null && (
                <div key="stage-change-modal" className="fixed inset-0 z-[100] flex items-end justify-center px-4 pb-12 sm:items-center sm:p-0">
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    <motion.div
                        ...
                        className="relative bg-card-dark border border-white/10 rounded-3xl p-8 w-full max-w-sm shadow-2xl overflow-hidden"
                    >
                        ...
                        <div className="flex flex-col items-center text-center">
                            ...
                            <p className="text-slate-400 text-sm mb-8 leading-relaxed">
                                You are moving this ticket back to <span className="text-primary font-bold uppercase tracking-widest text-[11px]">{SERVICE_STAGES[targetStageIndex]}</span>. This will notify the client.
                            </p>
                            ...
                            <div className="grid grid-cols-2 gap-4 w-full">
                                ...
                                <button
                                    onClick={onConfirm}
                                    className="h-14 bg-primary rounded-xl font-bold uppercase text-[10px] tracking-widest text-white shadow-lg shadow-primary/20 hover:brightness-110 transition-all"
                                >
                                    Confirm
                                </button>
                            </div>
